...
public class Game9x9Controller implements Initializable {
    public Pane pane;
    ...
    public GameModel g9x9Model = new GameModel(9, Client1.servercomm);
    ...
    public void StartButtonAction(ActionEvent actionEvent) {
        ...
        if (Game.Playercolor.equals(Color.BLACK)) {
            ...
            service.setOnSucceeded(event -> {
                ...
                g9x9Model.createrect(pane,table, label);
            });
        } else {
            GameModel.service service = new GameModel.service();
            service.start();
            service.setOnSucceeded(event -> {
                ...
                for (int i = 0; i < 9; i++)
                    for (int j = 0; j < 9; j++) {
                        g9x9Model.rectTable[i][j].rect.setVisible(false);
                    }
                ...
                service1.setOnSucceeded(event1 -> {
                    ...
                    if(answ.equals("OPPONENT_LEFT_GAME")){
                        g9x9Model.InfoWindow("Opponent left the game", passbutton);
                    }
                    if(answ.equals("OPPONENT_RESIGNED")){
                        g9x9Model.InfoWindow("Opponent resigned", passbutton);
                    }
                    if(answ.equals("OPPONENT_PASSED")){
                        for (int i = 0; i < 9; i++)
                            for (int j = 0; j < 9; j++) {
                                g9x9Model.rectTable[i][j].rect.setVisible(true);
                            }
                        passbutton.setDisable(false);
                        resignbutton.setDisable(false);
                    }
    ...
    public void onpassaction(ActionEvent actionEvent) {
        g9x9Model.sendpassed(pane, table,label);
    }
    ...
    public void ProposeTerritory(ActionEvent actionEvent) {
        g9x9Model.sendterritory(pane, table, label);
    }
    ...
    public void RejectTerritory(ActionEvent actionEvent) {
        g9x9Model.rejectterritory(pane, table);
    }
    ...
    public void AcceptTerritory(ActionEvent actionEvent) {
        g9x9Model.acceptterritory(table, label);
    }
    ...
    public void Resumegame(ActionEvent actionEvent) { g9x9Model.resume(pane,table,label);
    }
    ...
    public void resign(ActionEvent actionEvent) { g9x9Model.resign(pane, table);
    }
}