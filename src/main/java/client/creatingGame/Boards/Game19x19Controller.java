...
public class Game19x19Controller implements Initializable {
    public Pane pane;
    ...
    public GameModel g19x19Model = new GameModel(19, Client1.servercomm);
    ...
    public void StartButtonAction(ActionEvent actionEvent) {
        ...
        if (Game.Playercolor.equals(Color.BLACK)) {
            ...
            service.setOnSucceeded(event -> {
                ...
                g19x19Model.createrect(pane,table, label);
            });
        } else {
            GameModel.service service = new GameModel.service();
            service.start();
            service.setOnSucceeded(event -> {
                ...
                BlackName.setText(split[1]);
                    ...
                    for (int i = 0; i < 19; i++)
                        for (int j = 0; j < 19; j++) {
                            g19x19Model.rectTable[i][j].rect.setVisible(false);
                        }
                GameModel.service1 service1 = new GameModel.service1();
                    ...
                    service1.setOnSucceeded(event1 -> {
                        ...
                        if(answ.equals("OPPONENT_LEFT_GAME")){
                            g19x19Model.InfoWindow("Opponent left the game", passbutton);
                        }
                        if(answ.equals("OPPONENT_RESIGNED")){
                            g19x19Model.InfoWindow("Opponent resigned", passbutton);
                        }
                        if(answ.equals("OPPONENT_PASSED")){
                            for (int i = 0; i < 19; i++)
                                for (int j = 0; j < 19; j++) {
                                    g19x19Model.rectTable[i][j].rect.setVisible(true);
                                }
                            passbutton.setDisable(false);
                            resignbutton.setDisable(false);
                        }
    ...
    public void onpassaction(ActionEvent actionEvent) {
        g19x19Model.sendpassed(pane, table,label);
    }
    ...
    public void ProposeTerritory(ActionEvent actionEvent) {
        g19x19Model.sendterritory(pane, table, label);
    }
    ...
    public void RejectTerritory(ActionEvent actionEvent) {
        g19x19Model.rejectterritory(pane, table);
    }
    ...
    public void AcceptTerritory(ActionEvent actionEvent) {
        g19x19Model.acceptterritory(table, label);
    }
    ...
    public void Resumegame(ActionEvent actionEvent) { g19x19Model.resume(pane,table,label);
    }
    ...
    public void resign(ActionEvent actionEvent) { g19x19Model.resign(pane, table);
    }
}
